...
contract WireOFT is Script  {
    ...
    function run() external {
        ...
        // 
        uint master_chainId = vm.parseJsonUint(configData, string(abi.encodePacked(path, ".master_chainId")));
        ...
        string memory filenameRemote = string(
            abi.encodePacked("./scripts/DeploymentFoundry/deployed_addresses_", vm.toString(master_chainId), ".json")
        );
        ...
        address oft_address = vm.parseJsonAddress(deployedData2, ".numaOFT");
        ...
        IOAppSetPeer localOApp = IOAppSetPeer(oft_address);
        IOAppSetPeer remoteOApp = IOAppSetPeer(adapter_address);
        uint32 remoteEid = uint32(master_endpoint);
